    ...
    return(
        <div className='ProductSearch'>
            <Row>
                ...
                <Form.Group className="mb-3 d-flex align-items-end" as={Col} xs={6} md='auto'>
                    <Form.Check size='sm' type="checkbox" label="Instock" onChange={ e => setInStock(e.target.checked) }/>
                    <Form.Check size='sm' type="checkbox" label="Sale" onChange={ e => setOnSale(e.target.checked) }/>

                    <Button  size='sm' variant="primary" type="button" as={Link} to ='/shop/search/filter' onClick={() => props.submitSearch(selectedCollection, searchText, inStock, onSale)} style={{marginLeft: '15px'}}>
                        Search
                    </Button>
                    <Button  size='sm' variant="primary" type="button" as={Link} to ='/shop/search/filter' onClick={() => props.submitSearch({title: 'all', id: 'all'} , '', false, false)} style={{marginLeft: '5px'}}>
                        Cancel
                    </Button>
                </Form.Group>
            </Row>
